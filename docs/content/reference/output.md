...
describes what users get. Pick a format with `-o`, or let osrm choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
osrm <command> -o table   # aligned columns for reading
osrm <command> -o jsonl   # one JSON object per line, for piping
osrm <command> -o json    # a single JSON array
osrm <command> -o csv     # spreadsheet friendly
osrm <command> -o tsv     # tab-separated
osrm <command> -o url     # just the URL column
osrm <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
osrm <command> --fields id,title,url
```
...
```bash
osrm <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
osrm <command>            # a table, because this is a terminal
osrm <command> | wc -l    # JSONL, because this is a pipe
```
